...
class Notifier:
    ...
    async def notify_reservation_start(self, slot: Slot):
        msg = f"""
            *Оновлення:* 📅 Знайдено талон на дату *{slot.ch_date} {slot.ch_time}*. Запустив процес бронювання часу...
        """
        await self.tg_bot.send_message(
            chat_id=self.chat_id,
            text=msg,
    ...
    async def notify_reservation_approved(self, slot: Slot):
        msg = f"""
            *Оновлення:* 📅 Талон на здачу практичного іспиту (механічна коробка передачі) на машині сервісного центру МВС успішно взято на дату *{slot.ch_date} {slot.ch_time}*. Щасти на іспиті!
        """
